...
function mkEvt(state,{case_id,who,title_es}){
  return { state, case_id, who, title_es, ts: new Date().toISOString() };
}
...
export default async function handler(req,res){
  ...
  if(!acquireCaseLock(case_id)){
    res.statusCode = 409;
    return res.end(JSON.stringify({ ok:false, error:'already-picked', case_id }));
  }

  const evt = mkEvt('picked_up', body);
  await append(evt);
  await slackNotify(process.env.SLACK_WEBHOOK_URL, {text:`📌 Caso ${evt.case_id} recogido por ${evt.who}`});
