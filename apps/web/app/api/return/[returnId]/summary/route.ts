...
export async function GET(_: NextRequest, { params }: { params: { id: string } }) {
  ...
  const answers = await prisma.answer.findMany({ where: { returnId: params.returnId } })
  ...
  const openUnsureCount = await prisma.unsureFlag.count({ where: { returnId: params.returnId, status: "open" } })
