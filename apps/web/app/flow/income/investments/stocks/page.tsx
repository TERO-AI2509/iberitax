...
export default function Page(){
  ...
  function onSave(fd:FormData){
    ...
    const hasFile=Boolean(fd.get("stock_docs"));
    setData("income/investments/stocks",{ proceeds, cost, fees });
    setStatus("income/investments/stocks", hasAmount ? "entered" : hasFile ? "uploaded" : "pending");
    setSaved(true);
  }
  return (
    <form action={(fd)=>onSave(fd)} className="space-y-6">
      <h2 className="text-xl font-semibold">Investments — Stocks</h2>
