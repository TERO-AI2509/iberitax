...
function flatten(nodes: Node[], parent?: string): Node[] {
  return nodes.flatMap((n) => [{ ...n, parent }, ...(n.children ? flatten(n.children, n.key) : [])]);
}
...
export default function BranchNavControls({ clientId, returnId, nodeKey }: Props) {
  ...
  useEffect(() => { fetchTree(returnId).then(t => setFlat(flatten(t))); }, [returnId]);

  const idx = nodeKey
    ? flat.findIndex((n) => n.key === nodeKey)
    : flat.findIndex((n) => pathname?.endsWith(n.path));

  if (nodeKey && idx === -1 && flat.length) {
    // helps debug mismatched keys
    console.warn("BranchNavControls: nodeKey not found", { nodeKey, available: flat.map(n => n.key) });
  }
