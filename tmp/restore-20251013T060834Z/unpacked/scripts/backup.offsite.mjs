...
async function slackNotify(url, summary){
  ...
  if (summary?.offsite?.index) fields.push({type:'mrkdwn', text:`*Index:* <${summary.offsite.index}|open>`})
  if (!ok && summary.error) fields.push({type:'mrkdwn', text:`*Error:* ${String(summary.error).slice(0,400)}`})

  const payload = {
    text: `${ok ? 'SUCCESS' : 'FAIL'} · ${summary.name||''} · ${summary.mode||''}`,
    blocks: [
      { type:'header', text:{ type:'plain_text', text:`${ok?'✅':'❌'}  ${title}` } },
...
async function main(){
  ...
  const zip = path.join(outDir, zips[zips.length-1])
...
main().catch(async err=>{
  const msgObj = {ok:false, error:String(err.message||err)}
  await slackNotify(process.env.NOTIFY_WEBHOOK, msgObj)
  console.error(JSON.stringify(msgObj))
  process.exit(1)
})
